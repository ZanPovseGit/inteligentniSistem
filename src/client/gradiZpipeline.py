...
with mlflow.start_run(run_name="Grajenje modela2"):
    ...
    def load_and_process_data(json_file, selected_columns):
        ...
        print(data_selected.head())  # Print the first few rows of the selected columns
        ...
        # Get the column names generated by the DateTimeTransformer
        all_columns  = ['target', 'rain','day', 'month', 'year', 'hour', 'minute']

        df_lstm = pd.DataFrame(X_scaled, columns=all_columns)

        # Include the target column in the DataFrame
        lstm_target = 'available_bike_stands'
        df_lstm[lstm_target] = data[lstm_target]

        print("Data after transformation:")
        print(df_lstm.head())  # Print the first few rows of the transformed data

        y_lstm = data[lstm_target]  # Keep the target column as it is
    ...
    mlflow.tensorflow.save_model(lstm_model, "models")


    try:
        previous_run = mlflow.search_runs(order_by=["metric.accuracy DESC"]).iloc[0]
        previous_run_id = previous_run.run_id
        previous_model_path = f"runs:/{previous_run_id}/models"
        previous_model = mlflow.tensorflow.load_model(previous_model_path)
        previous_accuracy = previous_run["metrics.accuracy"]
    ...
    if not previous_run or new_accuracy > previous_accuracy:
        mlflow.tensorflow.save_model(lstm_model, "models")
